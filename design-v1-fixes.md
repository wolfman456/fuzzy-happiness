...
| # | Issue | Severity | Priority | Status | Resolution |
|---|-------|----------|----------|--------|------------|
| #85 | Compiled sheet shows **no subclass features for curated-only archetypes**: `collectedFeatures` only merges SRD-listed subclass levels (`CharacterService.java:258-260`); curated subclasses (College of Glamour, most Tasha's/XGtE options) contribute nothing, and `feature_choices` are unmodeled (design-draft-v2 §7) | medium | P2 | 🐛 open | filed as v2.0 hot-fix issue (before slices) |
| #87 | Ability-score step: rerolls should be capped at **2 max** — both the full "Roll all again" budget (currently 1) and each per-ability re-roll (currently uncapped) (design-v1 §8) | medium | P3 | 🔧 fixing | `feature/reroll-2-max` |
