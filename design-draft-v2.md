...
- **Slice 3 editor surface (decided 2026-09-14):** a **dedicated full-screen map-editor route**,
  ...
  iteration.
- **Chargen bugs (decided 2026-09-14):** Bug A and the **residual Bug B** are filed as **v2.0
  hot-fixes before any slice** (Bug A shipped in v1.1 as #58 and is verified fixed — re-open
  only the unreleased piece: subclass features for curated-only subclasses + `feature_choices`).
...
- **Bug B — class/subclass/race features missing.** ✅ **Fix shipped in v1.1 (#58)** — subclass +
   race features merge at compile and the sheet + Review render them — **but partially: the
   residual gap is now the v2.0 hot-fix** (see §2). `collectedFeatures` merges class level-ups +
   race traits always, yet subclass features only arrive for **SRD-listed** archetypes
   (`CharacterService.java:258-260`); curated-only subclasses (College of Glamour, most Tasha's/
   XGtE options) still contribute zero subclass features, and `feature_choices` are unmodeled.
   Tracked as a filed hot-fix issue before slices.
